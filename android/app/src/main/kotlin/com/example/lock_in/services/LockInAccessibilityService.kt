...
class LockInAccessibilityService : AccessibilityService() {
    ...
    private fun checkAppLimit(packageName: String) {
        try {
            if (appLimitManager.hasExceededLimit(packageName)) {
    ...
    private fun blockYouTubeShortsWithOverlay() {
        try {
            Log.d(TAG, "🎯 YouTube Shorts detected - navigating to Home first, then showing overlay")
            
            // STEP 1: Navigate to home immediately while YouTube is still accessible
            val navigationSuccess = navigateToYouTubeHomeImmediate()
            
            // STEP 2: Show brief success/info overlay after navigation
            Handler(Looper.getMainLooper()).postDelayed({
                if (navigationSuccess) {
                    showSuccessNavigationOverlay()
                } else {
                    showAutoClosingShortsOverlay() // Fallback overlay if navigation failed
                }
            }, 200) // Very brief delay just to let navigation complete
            
        } catch (e: Exception) {
            Log.e(TAG, "Error showing YouTube Shorts overlay", e)
            // Fallback to regular overlay
            showShortFormBlockOverlay("com.google.android.youtube")
        }
    }
    ...
    private fun showShortsBlockingOverlay() {
        try {
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                ...
                putExtra("MESSAGE", "🚫 YouTube Shorts Blocked\n\n📱 You can still navigate to other YouTube sections\n\n🏠 Tap the Home tab below to watch regular videos")
                putExtra("IS_STRICT_MODE", false)
                putExtra("ALLOW_NAVIGATION", true)
                putExtra("SHOW_GUIDANCE", true)
            }
    ...
    private fun showWebsiteBlockOverlay(url: String, appName: String) {
        try {
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                ...
                putExtra("MESSAGE", "🚫 Website Blocked\n\n$url\n\nThis website is blocked by your focus settings.")
    ...
    private fun showShortFormBlockOverlay(packageName: String) {
        try {
            ...
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                ...
                putExtra("APP_NAME", appName)
                putExtra("MESSAGE", "🎯 Short-form content blocked\n\n$appName\n\nStay focused! Short-form content can be distracting.")
                putExtra("IS_STRICT_MODE", false)
            }
            startActivity(intent)
        } catch (e: Exception) {
    ...
    private fun showBrowserShortsBlockOverlay(packageName: String, url: String) {
        try {
            ...
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                ...
                putExtra("APP_NAME", appName)
                putExtra("URL", url)
                putExtra("MESSAGE", "🎯 Short-form content blocked\n\n$url\n\nThis type of content is blocked to help you stay focused.")
                putExtra("IS_STRICT_MODE", false)
            }
            startActivity(intent)
        } catch (e: Exception) {
    ...
    private fun showAutoClosingShortsOverlay() {
        try {
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                ...
                putExtra("PACKAGE_NAME", "com.google.android.youtube")
                putExtra("APP_NAME", "YouTube")
                putExtra("MESSAGE", "🚫 YouTube Shorts Blocked\n\n🏠 Navigating to Home tab...\n\n✨ Stay focused with regular videos!")
                putExtra("IS_STRICT_MODE", false)
                putExtra("AUTO_CLOSE", true)
                putExtra("AUTO_CLOSE_DELAY", 2500L) // Auto close after 2.5 seconds
                putExtra("SHOW_GUIDANCE", true)
            }
            startActivity(intent)
        } catch (e: Exception) {
    ...
    private fun showSuccessNavigationOverlay() {
        try {
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                ...
                putExtra("PACKAGE_NAME", "com.google.android.youtube")
                putExtra("APP_NAME", "YouTube")
                putExtra("MESSAGE", "✅ YouTube Shorts Blocked\n\n🏠 Navigated to Home tab!\n\n✨ You can now watch regular videos")
                putExtra("IS_STRICT_MODE", false)
                putExtra("AUTO_CLOSE", true)
                putExtra("AUTO_CLOSE_DELAY", 2000L) // Auto close after 2 seconds
                putExtra("SHOW_GUIDANCE", false) // Success message, no guidance needed
            }
            startActivity(intent)
        } catch (e: Exception) {
    ...
    private fun showAppLimitExceededOverlay(packageName: String) {
        try {
            val appName = getAppName(packageName)
            val limit = appLimitManager.getAppLimit(packageName) ?: 0
            val used = appLimitManager.getTodayUsage(packageName)
            
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                flags = Intent.FLAG_ACTIVITY_NEW_TASK
                putExtra("PACKAGE_NAME", packageName)
                putExtra("APP_NAME", appName)
                putExtra("MESSAGE", "⏰ Daily limit reached\n\n$appName\n\nYou've used $used of $limit minutes today.\n\nTake a break and come back tomorrow!")
                putExtra("IS_STRICT_MODE", true)
            }
            startActivity(intent)
        } catch (e: Exception) {
    ...
    private fun blockInstagramReelsWithOverlay() {
        try {
            Log.d(TAG, "📸 Instagram Reels detected - navigating to Home first, then showing overlay")
            
            // STEP 1: Navigate to Instagram Home immediately while accessible
            val navigationSuccess = navigateToInstagramHomeImmediate()
            
            // STEP 2: Show brief success/info overlay after navigation
            Handler(Looper.getMainLooper()).postDelayed({
                if (navigationSuccess) {
                    showSuccessInstagramNavigationOverlay()
                } else {
                    showAutoClosingInstagramOverlay() // Fallback overlay if navigation failed
                }
            }, 200) // Very brief delay just to let navigation complete
    ...
    private fun showSuccessInstagramNavigationOverlay() {
        try {
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                ...
                putExtra("title", "Instagram Reels Blocked")
                putExtra("message", "📸 Instagram Reels are blocked to help you stay focused.\n\nYou've been redirected to your Home feed.")
                putExtra("isShortFormBlock", true)
                putExtra("platform", "Instagram")
                putExtra("AUTO_CLOSE", true)
                putExtra("AUTO_CLOSE_DELAY", 3000L) // 3 seconds auto-close
            }
            startActivity(intent)
            
            Log.d(TAG, "✅ Instagram success navigation overlay shown")
        } catch (e: Exception) {
    ...
    private fun showAutoClosingInstagramOverlay() {
        try {
            val intent = Intent(this, BlockOverlayActivity::class.java).apply {
                flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
                putExtra("title", "Instagram Reels Blocked")
                putExtra("message", "📸 Instagram Reels are blocked to help you stay focused.\n\nPlease navigate to your Home feed manually.")
                putExtra("isShortFormBlock", true)
                putExtra("platform", "Instagram")
                putExtra("AUTO_CLOSE", true)
                putExtra("AUTO_CLOSE_DELAY", 4000L) // 4 seconds auto-close
            }
            startActivity(intent)
            
            Log.d(TAG, "📸 Instagram auto-closing overlay shown")
        } catch (e: Exception) {
