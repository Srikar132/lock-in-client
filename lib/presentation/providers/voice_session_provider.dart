...
final voiceSessionProvider = NotifierProvider<VoiceSessionNotifier, VoiceSessionStateModel>(() {
...
class VoiceSessionNotifier extends Notifier<VoiceSessionStateModel> {
  ...
  // Safe state update
  void _updateState(VoiceSessionStateModel Function(VoiceSessionStateModel) update) {
    if (_mounted) {
      try {
        state = update(state);
      } catch (e) {
  ...
  Future<bool> initialize() async {
    try {
      ...
      // Transcript received
      _transcriptSubscription = _realtimeService.transcriptStream.listen((transcript) {
      ...
      _responseSubscription = _realtimeService.responseStream.listen((response) {
        print('💬 Got response text: $response');
        _updateState((s) => s.copyWith(partialResponse: response));
      });

      // Audio chunks - just queue them
      _audioResponseSubscription = _realtimeService.audioResponseStream.listen((chunk) {
      ...
      _stateSubscription = _realtimeService.stateStream.listen((apiState) {
        ...
        switch (apiState) {
          ...
          case 'response_complete':
            ...
            if (state.partialResponse != null && state.partialResponse!.isNotEmpty) {
              final message = VoiceMessage(
                id: _uuid.v4(),
                role: 'assistant',
